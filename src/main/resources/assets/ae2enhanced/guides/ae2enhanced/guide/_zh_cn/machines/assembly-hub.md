...
- **时间折叠模块** (并行, 最多 5 张): 批量合成并行上限. 0 张 = 64, 每张乘 32, 封顶 67,108,864; 装 5 张时 tooltip 显示无穷.
- **时空膨胀模块** (速度, 最多 5 张): 批量合成冷却从 20 tick 起, 每张减半, 最低 1 tick.
- **能量优化模块** (效率): tooltip 声明降低枢纽能耗.
- **维度折叠模块** (容量): 见上方样板槽说明.
...
枢纽借助稳定黑洞合成时, 3x3x3 事件视界内的物品会被吸入作为原料, 范围内生物被秒杀 (伤害类型 "spacetime", 每 5 tick 节流一次). 最多支持 5 种溢出物品类型.
