...
1. 通过[黑洞合成](systems/black-hole.md)获得 <ItemLink id="smart_blank_pattern" /> **智能空白样板** (向事件视界投入 64 个空白样板). ME 接口会忽略未编码的空白样板.
2. 在 <ItemLink id="smart_pattern_interface" /> **智能样板接口**的 GUI 中编码. 可以从 JEI 拖拽 ghost 物品到接口中选择配方.
...
- 可将机器列入黑名单禁止使用智能样板 (`smartPattern.blacklist`; 默认排除熔炉与基础合成拓展工作台).
