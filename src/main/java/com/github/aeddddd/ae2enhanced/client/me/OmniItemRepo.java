...
public class OmniItemRepo extends ItemRepo {
        ...
        for (PacketOmniInventoryUpdate.Entry e : entries) {
            ...
            try {
                ...
                com.github.aeddddd.ae2enhanced.AE2Enhanced.LOGGER.error(
                        "[AE2E] Failed to register item in handleFullInit: id={}, stack={}", e.id, e.stack, ex);
            }
        }
    ...
    public void handleFullContinue(List<PacketOmniInventoryUpdate.Entry> entries) {
        for (PacketOmniInventoryUpdate.Entry e : entries) {
            try {
                ...
                com.github.aeddddd.ae2enhanced.AE2Enhanced.LOGGER.error(
                        "[AE2E] Failed to register item in handleFullContinue: id={}, stack={}", e.id, e.stack, ex);
            }
        }
        // 每批 FULL_CONTINUE 后都更新 flatList，避免 FULL_END 丢失时客户端长期只有 500 个物品
        syncFlatList();
        setChanged(true);
    ...
    public void syncFlatList() {
        ...
        // 强制下次 computeView 全量重建，否则 needRebuild=false 会导致只复用旧 view
        this.viewSnapshot.view.clear();
        this.viewSnapshot.viewMode = null;
    }
    ...
    private void rebuildIndex() {
        ...
        for (int i = 0; i < this.flatList.size(); i++) {
            ...
            if (modId != null) {
                ...
                String rawMid = Platform.getModId(stack);
                String mid = rawMid != null ? rawMid.toLowerCase() : "unknown";
    ...
    private void computeView() {
        try {
            ...
            synchronized (this.flatListLock) {
                ...
                snapshotVersion = this.flatListVersion;
            }
            ...
            // 检查 flatList 是否在计算期间变化了（如 FULL_END 期间 syncFlatList）
            if (snapshotVersion != this.flatListVersion) {
                // 放弃过时的结果，重新提交计算
                this.viewUpdatePending.set(false);
                this.updateView();
                return;
            ...
            // 双缓冲原子替换
            this.renderView = newView;
            this.renderViewVersion = snapshotVersion;
    ...
    private boolean matchesName(IAEItemStack is) {
        String rawName = searchCache.isModSearch ? Platform.getModId(is) : Platform.getItemDisplayName(is);
        String dspName = rawName != null ? rawName.toLowerCase() : "";
